...
class DQN:
    ...
    def replay(self):
        ...
        Q_futures = np.amax(self.target_model.predict(new_states), axis=1)
        #targets = self.model.predict(states)
        #Q_futures = np.amax(self.model.predict(new_states), axis=1)
        targets[np.arange(0,32),actions] = rewards + Q_futures*self.gamma*(1-dones)
        loss_hist = self.model.fit(states, targets, epochs=1, verbose=0)
        return [loss_hist.history['loss'][0], np.mean(Q_futures), np.mean(targets)]
